...
def get_args():
    ...
    parser.add_argument('--train_coor_dist', default=False, action='store_true', help="Whether to train nor mal feature distribution from position information")
...
if __name__ == '__main__':
    ...
    if args.train_coreset :
        ...
        coreset_generator_trainer = pl.Trainer(default_root_dir=os.path.join(default_root_dir, 'coreset'), max_epochs=1, gpus=1, enable_checkpointing=False)
    ...
    if args.train_nb_dist:
        ...
        distribution_trainer = pl.Trainer(max_epochs=args.num_epochs, gpus=1, logger=tb_logger, log_every_n_steps=50, enable_checkpointing=False) 
    ...
    anomaly_calculator = pl.Trainer( default_root_dir=os.path.join(default_root_dir, 'anomaly'), max_epochs=1, gpus=1, enable_checkpointing=False) #, check_val_every_n_epoch=args.val_freq,  num_sanity_val_steps=0) # ,fast_dev_run=True)
